...
function osen_wc_init(){
	...
	function osen_wc_add_shilling_symbol( $currency_symbol, $currency ) {
		switch( $currency ) {
			case 'KES': 
		...
		if(!class_exists('WC_Osen_PesaPal_Gateway')){
			...
			class WC_Osen_PesaPal_Gateway extends WC_Payment_Gateway{
				...
				public function admin_options() {
					...
					<p>
						<?php _e('PesaPal requires Full names and email/phone number. To handle IPN return requests, please set the url '); ?>
				...
				function payment_page($order_id){
					if(isset($_REQUEST['pesapal_merchant_reference'])){
						...
						<div class="pesapal_container" style="position:relative;">
							<img class="pesapal_loading_preloader" src="<?php echo OSEN_WC_PESAPAL_PLUGIN_URL; ?>/assets/img/loader.gif" alt="loading" style="position:absolute;"/>
